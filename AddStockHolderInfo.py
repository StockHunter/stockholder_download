...
global ENCODE
global SEED_WEB
SEED_WEB = "http://vip.stock.finance.sina.com.cn/corp/go.php/vCI_StockHolder/stockid/"
global RETRY_CNT
global countOK
global countNG
global DEBUG_LOG
...
def getWebInfo(url):
    ...
    error = 0
    cnt = 1
    while error == 0 and cnt < RETRY_CNT:
        error = -1
        try:
            req = urllib.request.Request(url, headers=Header)
            webInfo_orig = urllib.request.urlopen(req)
        except urllib.request.HTTPError as e:
            print("(Retry)Exception Code:%s Msg:%s !!" % (e.code, e.msg))
            if e.code >= 400 and e.code < 500:
                print("(Quit)Exception Code:%s Msg:%s !!" % (e.code, e.msg))
                print("(Quit)Downloading url:%s !!" % (url))
                return -1
            print("(Retry)Exception Code:%s Msg:%s" % (e.code, e.msg))
            print("Sleep 15 seconds. url:%s Retry_cnt:%d" % (url, cnt))
            time.sleep(15)
            cnt = cnt + 1
            error = 0
            if e.code == 11001:
                print("(Quit)Exception Code:%s Msg:%s !!" % (e.code, e.msg))
                return -1
    if webInfo_orig is None:
        print("(Quit)page is NULL")
        return -1
    if cnt == RETRY_CNT:
        print("(Quit)Retry_cnt Max !!")
        return -1
    print("urlopen Success! url:%s" % (url))
    html_tmp = webInfo_orig.read()
    print("read Success! url:%s" % (url))
    ###    html = html.decode('gb2312')
    webInfo_orig.close()
    return html_tmp

def decodeFunc(html_tmp):
    global ENCODE
    global stockholder_info_url
    ENCODE = chardet.detect(html_tmp)['encoding']
    print("%s <- %s" % (stockholder_info_url, ENCODE))
    html = html_tmp.decode(ENCODE, 'ignore')
    return html


def WriteFile(fname, data, encode):
    f = open(fname, 'a', encoding=encode)
    if f:
        f.write(data)
        f.close()
    else:
    ...
    for a in range(len(list1)):
        ...
        if len1%4 != 0:
            ...
            if DEBUG_LOG == 2:
                ...
                errString = "Count[" + str(a) + "] len[" + str(len1) + "]  code[" + str(LIST_ROWS[row][0]) + "]: " + str(list1[a])
            ...
            if DEBUG_LOG == 1:
                ...
                print("=============InputData End  ===============")
            countNG = countNG + 1
            print("countOK: %d,countNG:%d code:%d " % (countOK, countNG, LIST_ROWS[row][0]))
            break
        countOK = countOK + 1
        print("countOK: %d,countNG:%d code:%d " % (countOK, countNG, LIST_ROWS[row][0]))
        list1[a].append("OK")
...
def run():
    ...
    rows_len = len(LIST_ROWS)
    print("Stock_sum:", LIST_ROWS)
    startReadAndExc(1)
    conn.commit()
